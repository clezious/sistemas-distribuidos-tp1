...
El `client_id` es persistido en disco por el `input_gateway` para poder recuperarlo en caso de que el servicio falle y se reinicie. De esta forma, se puede garantizar que el `client_id` sea único y no se repita incluso frente a fallas del sistema.
